...
#             # Detect account title
#             title_match = re.search(r'(?:title|type)\s*:?\s*([a-zA-Z\s]{3,30})', row_str, re.IGNORECASE)
#             if title_match and self.account_title == "Operating Account":
#                 self.account_title = title_match.group(1).strip()
...
class CSVExcelParser(BaseParser):
    ...
    def _extract_metadata(self, df):
        ...
        for r_idx in range(min(15, len(df))):
            ...
            # Generic fallback: the CSV/Excel parser has no AI extraction to
            # fall back on, so if none of the ~12 known brands matched, try
            # to detect an institution name ("<Name> Bank" / "<Name> Credit
            # Union" / "<Name> N.A." etc.) directly from this header row.
            if self.bank_name == "Unknown Bank":
                guessed = self.guess_bank_name_generic(row_str, max_lines=1)
                if guessed:
